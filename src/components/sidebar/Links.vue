    ...
    <div v-if="links.data.length" class="links">
      ...
      <table class="table" @click="click">
        <tr v-for="obj in links.data" :key="obj" class="row">
          ...
          <td class="cell">
            <div class="wrapper">
              <select class="select" :value="obj.link.type" :data-id="obj.link.id" @change="select">
          ...
          <td>
            <div class="icon-delete" data-action="delete-link" :data-id="obj.link.id" />
          </td>
        </tr>
      </table>
    </div>
...
const props = defineProps(['linksData'])
